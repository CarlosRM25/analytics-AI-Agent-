...
- ✅ `app/store.py` — one pooled `redis.Redis` from `REDIS_URL`, or `None`. Every
  ...
  a Redis URL is set (and `DEPLOY_MODE=deployed` now requires one again).
- ✅ `app/cache.py` — `ResponseCache`: key `sha256(normalize(q)|dataset|model)`,
  value the JSON `/ask` payload, TTL `RESPONSE_CACHE_TTL_DAYS` (30). A hit is
  served verbatim with `cached: true` and **does not count against the limit**.
- ✅ `app/limits.py` — `RateLimiter`: two date-keyed Redis counters
  (`ratelimit:visitor:<ip_hash>:<date>`, `ratelimit:global:<date>`, 48h TTL).
  `check()` reads without incrementing; `record()` bumps both — called only after
  a novel, successful answer. `hash_ip(ip, RATE_LIMIT_SALT)`.
- ✅ `app/budget.py` — `MonthBudget`: sums `usage.cost_usd` into `budget:<YYYY-MM>`
  (~40d TTL); at `MONTHLY_BUDGET_USD` the API behaves as disabled until the UTC
  month rolls over.
- ✅ `app/api.py` `/ask` flow: `DEMO_ENABLED`/budget → 400 on empty →
  Turnstile (only if `TURNSTILE_SECRET` set) → cache lookup → rate-limit check →
  run → cache.put + limiter.record + budget.add. CORS locked to
  `CORS_ALLOWED_ORIGIN` (`@after_request` header + `OPTIONS /ask` → 204).
- ✅ `config.py` — `_strip_inline_comment_values` (a `mode="before"` validator):
  pydantic-settings keeps a trailing `# comment` as the value, so a blank-but-
  commented `.env` line was silently switching Turnstile on. Any value starting
  with `#` is now treated as unset. `.env.example` rewritten with comments on
  their own lines. `RATE_LIMIT_SALT` added.
- ✅ `frontend/index.html` — real offline-first page: intro, an **example
  gallery** rendered from `examples.json` (question + answer + SQL + chart, zero
  backend calls on load), an opt-in "Ask your own" reveal with suggestion chips,
  live answers rendered identically, and handling for `{disabled}` /
  `{limited, scope}` / `{cached}`. Minimal Markdown (GFM tables, headings, bold).
  `<meta name="api-base">` = "" for same-origin, or the Cloud Run URL for a split
  deploy.
- ✅ `frontend/examples.json` — 8 pre-run Q&A (2 with charts), generated by
  running the agent once in deployed mode; ~31 KB, committed.
- ✅ `deploy/warm_cache.py` — `python -m deploy.warm_cache https://URL` POSTs
  every `evals/questions.yaml` question (+ `--extra` file) to warm the cache.
- ✅ `deploy/cloudrun.yaml` — M8 env wired (`REDIS_URL` from a `redis-url`
  secret, `CORS_ALLOWED_ORIGIN`, `DEMO_ENABLED`, cap numbers).
  `deploy/MONITORING.md` — the account-level setup (Upstash, Console cap, GCP
  budget, UptimeRobot, kill switch) as a runbook.
- ✅ **Verified end-to-end against a real Redis container** (`redis:7-alpine`):
  cache hit on a whitespace/case variant, `cached` bypasses the limit, per-visitor
  429 `{limited, scope: "visitor"}` at the cap, counters + budget accumulate with
  correct TTLs (48h / 30d / 40d), a seeded over-budget key → `{disabled: true}`,
  CORS preflight + header correct. 105 tests (`fakeredis` for the unit tests).
- ⚠️ **Not deployed** — no GCP project/billing yet; `MONITORING.md` + `cloudrun.yaml`
  are the runbook for when it exists. The frontend host (portfolio site vs Pages)
  is still open per §12.

**M7 — containerize + deploy, complete** (merged).
